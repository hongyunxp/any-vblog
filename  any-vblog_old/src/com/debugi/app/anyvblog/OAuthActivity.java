...
import com.debugi.app.anyvblog.adapter.AccountAdapter;
import com.debugi.app.anyvblog.adapter.OAuthSpAdapter;
...
import com.debugi.app.anyvblog.oauth.RetrieveAccessTokenTask;
...
public class OAuthActivity extends Activity implements OnClickListener {
					...
					try {
						...
						if (response == null || response.getCode() != 200) {
							...
							String tip = new ResponseException(response,
									sp).getMessage();
							Toast.makeText(activity, "程序出现异常" + tip, Toast.LENGTH_SHORT)
									.show();
							return;
						}
						parseData();
	...
	public void onClick(View v) {
		...
		case R.id.startOauth:
			Toast.makeText(activity, "你将要前往服务商授权页面,此过程持续时间和网络相关,请耐心等待", Toast.LENGTH_LONG).show();
			String url;
			switch (sp) {
			case Constants.SP_TENCENT:
		...
		case R.id.btn_home:
			if (users == null || users.size() <= 0) {
				Toast.makeText(activity, "还没有授权账户,请授权或退出", Toast.LENGTH_SHORT)
						.show();
				break;
			}
