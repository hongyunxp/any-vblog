...
import com.debugi.app.anyvblog.model.netease.GetUsers4Netease;
import com.debugi.app.anyvblog.model.netease.SendUsers4Netease;
import com.debugi.app.anyvblog.model.netease.UserInfo4Netease;
import com.debugi.app.anyvblog.model.sina.GetUsers4Sina;
import com.debugi.app.anyvblog.model.sina.SendUsers4Sina;
import com.debugi.app.anyvblog.model.sina.UserInfo4Sina;
import com.debugi.app.anyvblog.model.sohu.GetUsers4Sohu;
import com.debugi.app.anyvblog.model.sohu.SendUsers4Sohu;
import com.debugi.app.anyvblog.model.sohu.UserInfo4Sohu;
import com.debugi.app.anyvblog.model.tencent.GetUserInfos4Tencent;
import com.debugi.app.anyvblog.model.tencent.SendFriends4Tencent;
...
public class FriendsActivity extends Activity implements OnClickListener {
				...
				case MyHandler.Handler_GetData:
					...
					try {
						...
						if (response == null || response.getCode() != 200) {
							...
							String tip = new ResponseException(response, curUser.getSp()).getMessage();
	...
	private void getDataFromIntent(Intent intent) {
		...
		user_nick.setText(curUserNick + " 的好友");
	}
	...
	private void initViews() {
		...
		users_list.setOnClickFooterListener(new OnClickFooterListener() {
			...
			public void onClick() {
				...
				type = 1;
				sendRequest();
				users_list.onClickComplete(R.string.footer_loading_tips);
			}
		});
	}
	...
	private List<UserAdapter> parseData(Response response) {
		...
		try {
			...
			case Constants.SP_TENCENT:
				...
				// 若返回值ret为0，则解析到适配
				if (infos4Tencent != null && infos4Tencent.getData().getHasnext() == 0 && infos4Tencent.getRet() == 0 && infos4Tencent.getData().getInfo().size() > 0) {
					for (UserInfo4Tencent user : infos4Tencent.getData().getInfo()) {
			...
			case Constants.SP_SINA:
				GetUsers4Sina users4Sina = gson.fromJson(response.getBody(), GetUsers4Sina.class);
			...
			case Constants.SP_NETEASE:
				GetUsers4Netease users4Netease = gson.fromJson(response.getBody(), GetUsers4Netease.class);
				if (users4Netease != null && users4Netease.getUsers().size() > 0) {
			...
			case Constants.SP_SOHU:
				GetUsers4Sohu users4Sohu = gson.fromJson(response.getBody(), GetUsers4Sohu.class);
	...
	private SendData createSendData() {
		...
		switch (Config.currentUser.getSp()) {
		case Constants.SP_TENCENT:
			sendData = new SendFriends4Tencent(user_id, count, (curPage - 1) * count);
			url = Constants.Tencent.FRIENDS_LIST;
			break;
		case Constants.SP_SINA:
			sendData = new SendUsers4Sina(getNext(), count);
			url = String.format(Constants.Sina.FRIENDS_LIST, user_id);
			break;
		case Constants.SP_NETEASE:
			sendData = new SendUsers4Netease(user_id, getNext());
			url = Constants.NetEase.FRIENDS_LIST;
			break;
		case Constants.SP_SOHU:
			sendData = new SendUsers4Sohu(getNext(), count);
			url = String.format(Constants.Sohu.FRIENDS_LIST, user_id);
			break;
		default:
			break;
		}
